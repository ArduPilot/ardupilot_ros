...
class ArduPilotGUI(QMainWindow):
    ...
    def __init__(self, node):
        ...
        title_style = (
            "QLabel { padding: 10px; background-color: #2196F3; " "color: white; }"
        )
    ...
    def create_main_content(self, layout):
        ...
        self.connection_label = self._create_status_label(
            "Status: Connecting...", "#ffcccc"
        )
        ...
        self.arm_button = self._create_button(
            "ARM VEHICLE", "#4CAF50", self.arm_vehicle
        )
        self.disarm_button = self._create_button(
            "DISARM VEHICLE", "#f44336", self.disarm_vehicle
        )
        self.debug_button = self._create_button(
            "LIST TOPICS", "#2196F3", self.list_topics
        )
    ...
    def _create_prearm_status(self):
        ...
        prearm_style = (
            "QLabel { background-color: #fff3cd; padding: 8px; "
    ...
    def update_prearm_display(self):
        ...
        try:
            ...
            if (
                self.last_prearm_check
                and (current_time - self.last_prearm_check).total_seconds() > 10
            ):
                self.prearm_status_label.setText("Status: Data outdated")
                outdated_style = (
                    "QLabel { background-color: #f8d7da; padding: 8px; "
            ...
            if can_arm:
                ...
                ready_style = (
                    "QLabel { background-color: #d1e7dd; padding: 8px; "
    ...
    def _create_log_text(self):
        ...
        log_style = (
            "QTextEdit { background-color: #f5f5f5; " "border: 2px solid #ccc; }"
        )
    ...
    def _update_gps_display(self):
        ...
        if self.node.gps_data:
            ...
            gps_style = (
                "QLabel { background-color: #ccffcc; padding: 10px; "
    ...
    def _create_mode_buttons(self):
        ...
        self.mode_hold_btn = self._create_mode_button(
            "HOLD", lambda: self.set_mode("HOLD")
        )
        self.mode_guided_btn = self._create_mode_button(
            "GUIDED", lambda: self.set_mode("GUIDED")
        )
        self.mode_auto_btn = self._create_mode_button(
            "AUTO", lambda: self.set_mode("AUTO")
        )
        self.mode_manual_btn = self._create_mode_button(
            "MANUAL", lambda: self.set_mode("MANUAL")
        )
    ...
    def _create_copter_controls(self):
        ...
        # Land button
        self.mode_land_btn = self._create_mode_button(
            "LAND", lambda: self.set_mode("LAND")
        )
        land_style = (
    ...
    def _create_velocity_control(self):
        ...
        vel_label_style = (
            "QLabel { background-color: #e3f2fd; padding: 4px; "
