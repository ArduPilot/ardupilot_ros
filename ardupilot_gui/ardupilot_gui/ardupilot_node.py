...
class ArduPilotNode(Node):
    ...
    def status_callback(self, msg):
        ...
        try:
            ...
            new_status_data = {
                ...
                "vehicle_type": mavlinkutils.VEHICLE_TYPES.get(
                    msg.vehicle_type, "UNKNOWN"
                ),
            }
    ...
    def arm_vehicle(self):
        ...
        try:
            ...
            future.add_done_callback(lambda fut: self._handle_arm_response(fut, True))
    ...
    def disarm_vehicle(self):
        ...
        try:
            ...
            # Use async call instead of blocking
            future = client.call_async(request)
            future.add_done_callback(lambda fut: self._handle_arm_response(fut, False))
        ...
        try:
            ...
            if response is not None:
                self.get_logger().info(
                    f"Mode switch to {mode_string} response received"
                )
                if response.status:
        ...
        try:
            ...
            if response is not None:
                ...
                self.get_logger().error(
                    f"{action.capitalize()} request failed - no response"
                )
